...
-- HelperPayroll is optional. Keep its data behind the public companion API and
-- cache the result briefly so the HUD does not rebuild payroll mapping indexes
-- ten times on every rendered frame.
local payrollCache = {
    refreshAfter = -1,
    present = false,
    available = false,
...
local function getPayrollAPI()
    ...
    local companion = rawget(_G, "HelperPayroll")
    if type(companion) == "table" and type(companion.integrationAPI) == "table" then
        return companion.integrationAPI
    end
    return nil
end

local function refreshPayrollCache()
    local now = tonumber(g_time) or 0
    if now < (payrollCache.refreshAfter or -1) then
        return payrollCache
    end

    payrollCache.refreshAfter = now + 750
    payrollCache.rolesBySlot = {}

    local api = getPayrollAPI()
    local companion = rawget(_G, "HelperPayroll")
    payrollCache.present = api ~= nil or type(companion) == "table"
    payrollCache.available = false
    ...
    if type(api.getStatus) == "function" then
        ...
        payrollCache.available = true
    end

    if type(api.getRoleForSlot) ~= "function" then
        return payrollCache
    end
    ...
    for index = 1, slotCount do
        ...
        if ok and type(roleData) == "table" then
            ...
            if label ~= nil and tostring(label) ~= "" then
                payrollCache.rolesBySlot[slot] = tostring(label)
            end
        end
    end
...
local function compactAppearanceLabel(label, workerName)
    local text = tostring(label or "-")
    local worker = tostring(workerName or "")
    if worker ~= "" then
        local escaped = worker:gsub("([^%w])", "%%%1")
        local compact = text:gsub("^" .. escaped .. "%s*%-%s*", "", 1)
        if compact == text then
            compact = text:gsub("^" .. escaped .. "%s+", "", 1)
        end
        if compact ~= "" then text = compact end
    end
    return text
end
...
local function collectRows()
    ...
    local payroll = refreshPayrollCache()
    summary.payrollPresent = payroll.present == true
    ...
    for index, helper in ipairs(profiles) do
        ...
        if not isGuiHiddenProfile(helperName) then
            ...
            local workerDisplayName = getDisplayName(helper, index)
            local payrollRole = nil
            if payroll.present == true then
                payrollRole = payroll.rolesBySlot[slotLabel]
                if payrollRole == nil then
                    payrollRole = payroll.available
                        and hpI18n("hp_overlay_payroll_unassigned", "-")
                        or hpI18n("hp_overlay_payroll_waiting", "WAITING")
                end
            end
            ...
            table.insert(rows, {
                ...
                appearance = compactAppearanceLabel(getAppearanceLabel(helper, index), workerDisplayName),
...
local function getColumnKeys(rows, rowCount)
    local keys = {"slot", "worker"}
    local showRole = false
    for index = 1, rowCount do
        if rows[index].role ~= nil then
            showRole = true
            break
        end
    end
    if showRole then table.insert(keys, "role") end
...
local function getColumnHeaders()
    return {
        ...
        role = hpI18n("hp_overlay_header_role", "ROLE"),
